...
async function fetchPoolsFromGraphQL(): Promise<TablePool[]> {
  if (TOKENIZED_STOCK_ADDRESSES.length === 0) {
    return [];
  }

  console.log(`🏊 [Tokenized Stock Pools API] Fetching pools for ${TOKENIZED_STOCK_ADDRESSES.length} tokenized stocks...`);
  ...
  const poolPromises = TOKENIZED_STOCK_ADDRESSES.map(async (tokenAddress) => {
    try {
      ...
      if (result.errors && result.errors.length > 0) {
        console.error(`❌ [Tokenized Stock Pools API] GraphQL errors for token ${tokenAddress}:`, result.errors);
      }
      ...
      if (poolCount > 0) {
        console.log(`✅ [Tokenized Stock Pools API] Found ${poolCount} pools for token ${tokenAddress}`);
      }
      ...
      console.error(`❌ [Tokenized Stock Pools API] Error fetching pools for token ${tokenAddress}:`, err);
      return null;
    }
  });
  ...
  console.log(`✅ [Tokenized Stock Pools API] Successfully fetched pools for ${validPools.length} out of ${TOKENIZED_STOCK_ADDRESSES.length} tokenized stocks`);
...
export async function GET(request: NextRequest) {
  try {
    ...
    const pools = await fetchPoolsFromGraphQL();
